...
function ListaProfessores($id){
	...
	if($id == 0){
		$query = mysqli_query($conexao,"SELECT idProfessor, Nome FROM Professor ORDER BY idProfessor ASC") or die(mysqli_error($conexao));
	}else{
		$query = mysqli_query($conexao,"SELECT idProfessor, Nome FROM Professor WHERE idProfessor = " .$id . " ORDER BY idProfessor ASC") or die(mysqli_error($conexao));
	}
